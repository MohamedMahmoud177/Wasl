...
async function enterApp(profile) {
  ...
  await Promise.all([renderDashboard(), renderAllTable(), renderCustomers(), renderAgents(), renderPricingBoard()]);
  if (isOwner) await renderStaffDirectory();
}
...
/* ================= دليل الموظفين + إضافة/حذف موظف (مدير المكتب فقط) ================= */
async function renderStaffDirectory() {
  ...
  document.querySelector("#employeesTable tbody").innerHTML = (data || []).map(p => `
    <tr>
      ...
      <td><button class="mini-btn" onclick="deleteStaff('${p.id}')">حذف</button></td>
    </tr>`).join("");
}
